...
[[vk::push_constant]]
ConstantBuffer<Constants> pushC;
...
void main()
{
  ...
  for(int smpl = 0; smpl < NBSAMPLES; smpl++)
  {
    ...
    P.prd.seed = tea(DispatchRaysIndex().y * DispatchRaysDimensions().x + DispatchRaysIndex().x, pushC.frame);
  ...
  if(pushC.frame > 0)
  {
    ...
    float3  old_color = image[int2(DispatchRaysIndex().xy)].xyz; 
